// The servlet that deals with returning the post list published by a specific user 
...
public class GetMyPost extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try (ObjectInputStream ois = new ObjectInputStream(request.getInputStream()); 
			...
			if (cursor == -1)
				p = database.getMyNewPost(uid, postNum);
			else
				p = database.getMyMorePost(uid, cursor, postNum);
